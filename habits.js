/**
 * Módulo de Hábitos (Visual Limpo - Sem Favoritos)
 */
app.render_habits = (root) => {
    ...
    root.innerHTML = `
        <div class="grid">
            <div class="col-4 card">
                <h3>Novo Hábito</h3>
                <input id="hName" placeholder="Ex: Ler 10 min" style="margin-bottom:8px">
                <input id="hTags" placeholder="Tags (ex: Saúde)" style="margin-bottom:12px">
                <button class="btn" style="width:100%" onclick="app.addHabit()">Adicionar</button>
            </div>
    ...
    app.renderHabitsList = () => {
        ...
        if (!app.state.habits.length) {
            el.innerHTML = '<div class="muted text-center" style="padding:40px">Nenhum hábito rastreado.</div>';
            return;
        }

        el.innerHTML = app.state.habits.map(h => {
            ...
            const historyDots = last7Days.map(date => {
                ...
                return `<div title="${date}" style="width:8px; height:8px; border-radius:50%; background:${color};"></div>`;
            }).join('');

            const tagsHtml = (h.tags || []).map(t => `<span class="tag-pill" style="font-size:10px">#${t}</span>`).join('');
            ...
            <div class="card" style="display:flex; justify-content:space-between; align-items:center; padding:16px;">
                <div>
                    ...
                    <div style="margin-top:6px; display:flex; gap:6px;">${tagsHtml}</div>
                </div>
                ...
                <div style="display:flex; flex-direction:column; align-items:flex-end; gap:8px">
                    ...
                    <div style="display:flex; gap:10px; align-items:center">
                        <span class="muted" style="font-size:12px">Dias: ${Object.keys(h.streaks).length}</span>
                        <button class="btn ${doneToday?'':'ghost'}" onclick="app.toggleHabit('${h.id}')" style="min-width:80px">
                            ${doneToday ? '<i class="ph ph-check"></i> Feito' : 'Marcar'}
                        </button>
